...
from database_manger_profit import add_profit_listing
...
async def send_profit_listing_embed(channel, listing_data):
    ...
    try:
        ...
        try:
            success = add_profit_listing(listing_data, message.id)
            if success:
    ...
    try:
        ...
        try:
            existing = db_manager.execute_query(
                'SELECT id FROM profit_listings WHERE auction_id = %s' if db_manager.use_postgres else 
                'SELECT id FROM profit_listings WHERE auction_id = ?',
                (auction_id,),
                fetch_one=True
            )
